...
public class FPSManagerSetResultsUIAction : ActionBase
{
    ...
    public override void Execute(GameObject instigator = null)
    {
        ...
        string sp = o.screenPercentage == 100 ? string.Empty : $"{o.screenPercentage}% SP ({o.upsamplingMethod})";
        FPSManager fpsm = Manager.Get<FPSManager>(); 
        QualityText.text = $"{go.width}x{go.height} ({go.fullScreenMode}) {sp} - {QualitySettings.names[QualitySettings.GetQualityLevel()]} Quality";
        ...
        CPUInfo.gameObject.SetActive(false);
        RAMInfo.text = $"System Memory : { SystemInfo.systemMemorySize / 1000}GB";
        GPUInfo.text = GPUInfo.text = SystemInfo.graphicsDeviceName;
    }
}
